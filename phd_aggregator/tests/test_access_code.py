...
def gated(monkeypatch):
    monkeypatch.setenv("CIK_ACCESS_CODE", CODE)
    # Faithful to where this gate actually runs: the desktop shell serves the
    # sidecar over plain-HTTP loopback and sets CIK_COOKIE_SECURE=0, because a
...
def ungated(monkeypatch):
    monkeypatch.delenv("CIK_ACCESS_CODE", raising=False)
