...
python astra.py                 # full run -> astra_positions.{csv,json,html}
python astra.py --list-sources  # what's registered/enabled
python astra.py --list-fields   # installed field profiles (fields/*.yaml)
python astra.py --country Germany --country Japan
python astra.py --field astronomy          # explicit field profile
```

Open `astra_positions.html` in a browser: search, filter by country / source /
...
```bash
python astra.py --list-fields             # what's already installed
python astra.py --new-field marine_biology  # interactive wizard
# or hand-write one:
cp fields/template.yaml fields/marine_biology.yaml   # fully commented
$EDITOR fields/marine_biology.yaml
python astra.py --field marine_biology
```
...
python astra.py --find-supervisors --field economics --country Germany
python astra.py --find-supervisors --field computer_science --country Japan
python astra.py --find-supervisors --field biology --country United States
...
| File | Content |
|---|---|
| `astra_positions.csv/.json` | all kept positions, sorted by relevance |
| `astra_positions.html` | self-contained dashboard (no network calls) |
...
| `astra_positions_universities.csv` | department registry + fetch status |
...
The single `astra.py` is being split into modules under `core/`.
Step 1:
