...
# -*- coding: utf-8 -*-
"""
astra.py — aggregate open PhD positions in astronomy/astrophysics
=========================================================================
...
RUN:
    python astra.py                 # config.yaml + CONFIG block below
    python astra.py --self-test     # offline test of the pipeline
    python astra.py --list-sources  # show registered sources + status
    python astra.py --list-fields   # show installed field profiles
    python astra.py --new-field my_major   # scaffold YOUR field profile
    python astra.py --source euraxess --source nature_careers
    python astra.py --country Germany --country Japan
    python astra.py --field astronomy       # field profile (fields/*.yaml)
    python astra.py --find-supervisors --field ism --country Germany
                                             # rank likely PhD supervisors
                                             # (OpenAlex: any major, no token /
                                             # ADS for astro+physics / arXiv)
    python astra.py --seeds seeds.txt       # hand-picked position URLs
    python astra.py --proxy ""      # disable the proxy entirely
    python astra.py --extras        # + emails, professors, scholarships
    python astra.py --no-fetch --write-emails   # emails from last run
    python astra.py --no-fetch --find-professors --scholarships
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    ap.add_argument("--seed-db", metavar="PATH",
                    help="seed the opportunities table from the pipeline's "
                         "JSON output (e.g. astra_positions.json) and print the "
                         "record count")
...
def new_field_wizard(name: str) -> int:
    ...
    lines = [
        ...
        f"--new-field)",
        "# =============================================================================",
        "# Select at runtime with:  python astra.py --field " + name,
        "#",
        "# Matching rules (see the relevance engine in astra.py):",
    ...
    if _HAVE_YAML:
        ...
        return 1
    print(f"\nWrote {out}")
    print(f"Run:  python astra.py --field {name}")
    print(f"Re-run this wizard anytime for a new major:  "
          f"python astra.py --new-field another_major")
    return 0
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    print_header(os.environ.get("ASTRA_VERSION", ""))

    _load_dotenv()          # gitignored .env -> os.environ (ADS_API_TOKEN etc.)
    # After the .env load, so a file still written with the pre-Astra CIK_*
    # names configures the run instead of being silently ignored.
    from core.env import adopt_legacy_env
    for _legacy in adopt_legacy_env():
        logging.warning("%s is the old name for ASTRA_%s — rename it",
