...
def test_seed_from_json_matches_count(tmp_path):
    ...
    Base.metadata.create_all(eng)
    s = Session(eng)
    p = tmp_path / "astra_positions.json"
...
def test_resolve_db_url_default_and_env(monkeypatch):
    ...
    assert resolve_db_url() == "sqlite:///astra.db"
    monkeypatch.setenv("DATABASE_URL", "postgresql://u:p@localhost/astra")
    assert resolve_db_url() == "postgresql://u:p@localhost/astra"


def test_postgres_engine_uses_queue_pool(monkeypatch):
    monkeypatch.setenv("DATABASE_URL",
                       "postgresql://u:p@localhost:5432/astra")
