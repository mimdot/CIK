...
# failed. ``ASTRA_JOB_RETRIES`` is the number of *further* attempts after the
# first, ``ASTRA_JOB_RETRY_DELAY`` the backoff between them (seconds).
MAX_PIPELINE_RETRIES = max(0, int(os.environ.get("ASTRA_JOB_RETRIES", "2")))
RETRY_BACKOFF_S = max(0.0, float(os.environ.get("ASTRA_JOB_RETRY_DELAY", "2")))
