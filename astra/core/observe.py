...
FastAPI. JSON logging is opt-in via ``ASTRA_JSON_LOGS=1`` (default off keeps the
...
def configure_json_logging(level: int = logging.INFO,
    ...
    ``logger_names`` defaults to the app's loggers (``astra``,
    ...
    if _logger_configured:
        return
    targets = logger_names or ["astra", "uvicorn", "uvicorn.error",
