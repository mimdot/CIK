...
# `pyinstaller astra-api.spec` from the astra/ directory.
#
...
for module in CV_PARSERS:
    try:
        ...
        print(f"[astra-api.spec] WARNING: {module} is not installed — the "
...
exe = EXE(
    ...
    #
    # Off on Windows: there is no `strip` there, and PyInstaller only prints
    # "this option is ignored" for every binary it collects — a few hundred
    # lines of noise in the middle of the build log that reads like failure.
    strip=not IS_WINDOWS,
    # UPX compresses further if it is installed; harmless when it is not.
    #
    # Off on Windows, deliberately. Two separate reasons, both seen in the
    # wild rather than theorised:
    #   1. UPX rewrites the PE headers of the CRT and Python DLLs it packs
    #      (python3xx.dll, VCRUNTIME140.dll), and a bundle that was fine
    #      uncompressed then dies at startup with a bare 0xC0000005.
    #   2. A UPX-packed PE is one of the strongest heuristics Defender and
    #      SmartScreen have for "packed malware". The app is already
    #      unsigned; adding a packer on top turns a warning into a quarantine.
    # The size it would have saved is not worth either.
    upx=not IS_WINDOWS,
    upx_exclude=[],
    runtime_tmpdir=None,
    console=True,
